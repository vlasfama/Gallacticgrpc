	...
	launchers := []process.Launcher{
		...
		},
		{
			...
			Launch: func() (process.Process, error) {
				...
				//grpc.RegisterBlockChainServer(grpcServer, grpc.NewBlockchainService(bc))
				//grpc.RegisterNetworkServer(grpcServer, grpc.NewNetowrkService(nodeview))
				//grpc.RegisterTransactionServer(grpcServer, grpc.NewTransactor(transactor))
